...
public class SsrsteomTree
{
	...
	private void populateInformation()
	{
		...
		setCoeffecientOfHighestDegree(getTermintationEntity()
				/ (deltax * Math.pow(deltax, getHighestDegree()) * factorial(getHighestDegree())));
	}

	/**
	 * This takes the intial row and does the recursive deviation togenerate the
	 * tree
	 * 
	 * @param ssrsteomRow
	...
	/**
	 * getHighestDegree() is calling d d is suppose to be the degree of monomial
	 * This is representing f(x)=C_d*x^d
	 * 
	 * @param x
	 *            this represents an input value e.g. f(2) -> 5x^2 = 5(2^2) =(4)(5)
	 *            = returns 20
	 * @return
	 */
	 ...
	 * interpolation (finds the coeffecients of a given polynomial that will hit the
	 * input values)
	 * 
	 * @throws Exception
	 */
	...
	/**
	 * populates the coeffecient list with the solution (the polynomial that hits
	 * the input data)
	 */
	public void solve(SsrsteomTree localSsrsteomTree) throws Exception
	{
		// add the coeffecient of the current instace's highest degree into
		// this.coeffecientlist
		...
		/*
		 * populate successivePolynomialList (a list containining intial array for
		 * successive portion of the polynomial) with initial values (formed by
		 * subtracting the current instaces initial array from monimoial with thte same
		 * inputer value initial array)
		 */

		// TODO; change what is passed to highestDegreefunction with actual input values
		// instead of i+1
		for (int i = 0; i < localSsrsteomTree.getSsrsteomList().get(0).size(); i++)
		{
			SuccessivePoloynomialList.add(
					localSsrsteomTree.getSsrsteomList().get(0).get(i) - localSsrsteomTree.highestDegreeFunction(i + 1));
		}
		...
		/*
		 * base case if ssrsteomList is of size one (this means that ssrsteom list
		 * contains a list of a constant value e.g. [[2,2,2,2,]]) then run the solve
		 * method
		 */
